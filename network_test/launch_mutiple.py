...
def start_launches(num_devices, output_size, end_devices, train_matrix, test_matrix, train_true, test_true, epochs):
    ...
    for device_id in range(num_devices):
        ...
        end_device = EndDevice(str(device_id), output_size, '127.0.0.1', ports[device_id])#10203
...
def start_middle_launches(num_devices, output_size, train_true, test_true):
    ...
    devices = [2, 2, 3]
    for device_id in range(num_devices):
        middle_device = MiddleDevice(str(device_id), output_size, devices[device_id], '0.0.0.0', '127.0.0.1', ports[device_id], 10203)
...
def main(epochs: int, output_size: int):
    train_matrix, train_true, test_matrix, test_true, station_names, ordered_matrix, ordered_true_values, _, _ = get_dataset()
...
def listen_wait():
    ...
    try:
        server_socket.bind(('0.0.0.0', 12122))
